        ...
        <div className="container mx-auto px-4 max-w-7xl relative z-10">
          <div className="flex flex-col md:flex-row items-end justify-between gap-12">
            <div className="text-white max-w-3xl">
              ...
              <h1 className="text-4xl md:text-5xl font-extrabold mb-6 tracking-tighter leading-tight text-white">
                {brand.name} <br/><span className="text-gray-500 font-light text-white/50">Yetkili servisi</span>
              </h1>
            ...
            <div className="bg-white p-10 shrink-0 border border-white/20 rounded-[2.5rem] shadow-2xl">
              <img 
                src={brand.logo} 
                alt={brand.name} 
                className={`${brand.id === 'yilkar' ? 'h-[60px]' : 'h-16'} w-auto object-contain`} 
              />
            </div>
          </div>
          {/* Scroll Down Arrow */}
          ...
          <div className="lg:w-2/3">
                ...
                {brandServices.length > 0 ? brandServices.map(service => (
                  <div key={service.id} className="flex flex-col md:flex-row gap-8 bg-white border border-slate-100 rounded-[2.5rem] p-4 group hover:shadow-2xl hover:border-slate-200 transition-all duration-500">
                    ...
                    <div className="md:w-3/5 p-8 flex flex-col justify-center">
                      ...
                      <Link to={`/${service.id}`} className="inline-flex items-center text-[10px] font-black text-black tracking-tight mt-auto group-hover:text-amber-600 transition-colors">
            ...
            <section className="mb-24">
              <div className="flex justify-between items-end mb-12 border-b border-gray-200 pb-6">
                ...
                <Link to={`/${brand.id}/urunler`} className="text-[10px] font-black text-black tracking-tight hover:text-gray-600 transition-colors">Tümünü gör</Link>
              </div>
